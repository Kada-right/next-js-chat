...
export const getTokens = (tokens: number, timestamp: number, timeNow: number) => {
  ...
  if (numberOfDays === 1) {
    if (isMonday(timeNow)) {
      return Math.min(3, tokens + 2); //tokens + 2 > 3 ? 3 : tokens + 2;
    }
    return Math.min(3, tokens + 1); //tokens + 1 > 3 ? 3 : tokens + 1;
  }
